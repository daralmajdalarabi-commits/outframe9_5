...
const nextStatus: Record<string, 'in-progress' | 'completed'> = {
  'pending': 'in-progress',
    ...
    >
        ...
        <div className="flex items-center gap-1 ml-2">
          {isAdmin && item.status !== 'completed' && (
            <motion.button
              ...
              title={`Move to ${nextStatus[item.status] === 'in-progress' ? 'In Progress' : 'Completed'}`}
            >
      ...
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <span
          ...
          onClick={isAdmin && item.status !== 'completed' ? cycleStatus : undefined}
        >
